...
I write a webapp to do some Bayesian modeling with a `Julia` backend and thought that I should also try to fit the models using `Turing.jl` rather than do a `Python`
...
before doing the benchmarking. Here is what each script looks like.
...
Unit: milliseconds
 ...
 get_data() 18.78206 19.60983 20.10581 19.91485 20.23297 29.5218   100
```
...
BenchmarkTools.Trial: 100 samples with 1 evaluation per sample.
 Range (min … max):  5.433 ms …   8.141 ms  ┊ GC (min … max): 0.00% … 0.00%
 Time  (median):     5.724 ms               ┊ GC (median):    0.00%
 Time  (mean ± σ):   5.879 ms ± 461.050 μs  ┊ GC (mean ± σ):  0.00% ± 0.00%

   ▃  ▇█▄▃ ▂                                                   
  ▅██▃██████▆▇▃▅▆▆█▃▃▃▁▁▃▁▁▅▁▃▁▅▃▁▃▁▃▁▁▁▁▁▁▃▁▃▁▁▁▁▁▁▁▁▃▁▁▁▁▁▃ ▃
  5.43 ms         Histogram: frequency by time        7.61 ms <

 Memory estimate: 117.91 KiB, allocs estimate: 2829.
```
:::
...
Unit: milliseconds
        expr      min       lq     mean   median       uq      max neval
 get_ids(df) 9.954226 10.12694 10.69413 10.45248 11.16924 14.15869   100
```
...
```{.bash}
BenchmarkTools.Trial: 100 samples with 1 evaluation per sample.
 Range (min … max):  25.750 μs … 37.208 μs  ┊ GC (min … max): 0.00% … 0.00%
 Time  (median):     26.834 μs              ┊ GC (median):    0.00%
 Time  (mean ± σ):   27.056 μs ±  1.694 μs  ┊ GC (mean ± σ):  0.00% ± 0.00%

       ▅█                                                      
  ▆▆█▅▇██▅▅▁▁▃▁▁▁▁▃▃▁▁▁▁▁▁▁▃▁▁▁▁▁▁▁▃▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▁▃ ▃
  25.8 μs         Histogram: frequency by time        37.1 μs <
...
Unit: seconds
               expr     min       lq    mean   median       uq      max neval
 fit_model(df, ids) 7.38629 7.464158 7.57126 7.535486 7.625566 8.083352   100
```
